...
4. Synthesize an answer citing page links. Use `kiwi_backlinks` to find related pages.
...
2. `kiwi_changes` with `since=<last_checkpoint>` — review recent
   edits for quality.
3. For each issue:
   - Orphan page → add links from related pages or index to the orphan
...
- **No broken links.** Every wiki link should resolve to an existing page.
